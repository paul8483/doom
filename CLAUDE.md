...
Stages 0, 1, 2, 3, 4, and 5 of the plan are done; Stage 6 is split into sub-stages 6a–6f. Sub-stages 6a, 6b, and 6c are done; **6d (monster AI) is in progress** — pure-C# Tasks 1–6 and Unity glue Tasks 7–10 are done; PlayMode verification (Task 11) is WIP in the working tree.
...
- **Stage 6d (monster AI) — in progress.** Spec `docs/superpowers/specs/2026-07-03-monster-ai-design.md`; plan `docs/superpowers/plans/2026-07-03-monster-ai.md`. **Done (Tasks 1–10):** `MonsterDef`/`MonsterTable` (E1 roster) in `Doom.Things`; `MonsterRules`, `ChaseDir`, `MonsterBrain`+`IMonsterWorld` in `Doom.Game`; `NoiseAlert` in `Doom.Specials`; Unity glue — `SpriteBillboard.SetFrame`, `Projectile` (imp fireball), `MonsterController` (35 tics/s, capsule steps, sight, hitscan/melee/missile, door use via `LineActivator`), `DamageSource`+infighting in `EnemyHealth`, `ThingSpawner`/`MapLoader` wiring, `NoiseAlertSystem` (gunfire wakes heard sectors). On E1M1 monsters should wake, chase, attack, and die with animation when played in the editor. **WIP:** Task 11 PlayMode tests (`MonsterAiPlayTests.cs`, not yet committed — stabilize in a follow-up session). **Deferred:** XDEATH, flying monsters, sound (6f), A_Look chain-wake, ML_SOUNDBLOCK, visual sign-off (Task 12).

The master roadmap is in `docs/doom-unity-remake-plan.md` (written in Russian). Each stage has a paired design spec under `docs/superpowers/specs/` (`YYYY-MM-DD-<topic>-design.md`, the WHAT/WHY) and an implementation plan under `docs/superpowers/plans/` (`YYYY-MM-DD-<topic>.md`, task-by-task HOW): Stage 1 = `2026-05-28-wad-reader`, Stage 2 = `2026-05-28-geometry`, Stage 3 = `2026-05-29-player`, Stage 4 = `2026-05-30-textures`, Stage 6a = `2026-05-31-doors`, Stage 6b = `2026-05-31-player-damage`, Stage 6c = `2026-07-02-weapons`, Stage 6d = `2026-07-03-monster-ai` (spec adds the `-design` suffix). Stages 6a–6c are done; **6d Tasks 1–10 are done**; next concrete work is Task 11 (PlayMode tests) then 6e/6f per the roadmap.
...
The current test suite is **193 EditMode + 15 PlayMode** tests (plus 5 PlayMode `MonsterAiPlayTests` WIP, uncommitted):
...
- 31 EditMode from Stage 6d Tasks 1–6 (monster AI, pure C#): `MonsterTableTests` ×3, `MonsterRulesTests` ×4, `ChaseDirTests` ×6, `MonsterBrainWakeTests` ×7, `MonsterBrainCombatTests` ×8, `NoiseAlertTests` ×3 in `Doom.Game.Tests` / `Doom.Things.Tests` / `Doom.Specials.Tests`. PlayMode for 6d (Task 11) not yet in the committed suite.
