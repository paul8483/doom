...
Stages 0, 1, 2, 3, 4, and 5 of the plan are done; Stage 6 is split into sub-stages 6a–6f and 6a is done:
...
The master roadmap is in `docs/doom-unity-remake-plan.md` (written in Russian). Each stage has a paired design spec under `docs/superpowers/specs/` (`YYYY-MM-DD-<topic>-design.md`, the WHAT/WHY) and an implementation plan under `docs/superpowers/plans/` (`YYYY-MM-DD-<topic>.md`, task-by-task HOW): Stage 1 = `2026-05-28-wad-reader`, Stage 2 = `2026-05-28-geometry`, Stage 3 = `2026-05-29-player`, Stage 4 = `2026-05-30-textures`, Stage 6a = `2026-05-31-doors` (spec adds the `-design` suffix). Stage 6a (doors & interactive sectors) is done; Stage 6b (player damage & HP) is the next concrete work.
...
The current test suite is 124 EditMode + 5 PlayMode tests:
