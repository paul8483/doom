...
Stages 0, 1, and 2 of the plan are done:
...
The plan is in `docs/doom-unity-remake-plan.md` (written in Russian); Stage 1's detailed plan is in `docs/stage-1-wad-reader-plan.md`; Stage 2's detailed plan is in `docs/stage-2-geometry-plan.md`. Stage 3 (first-person player with collisions) is the next concrete work.
...
The current test suite is 62 EditMode tests:
...
- 32 from Stage 2 (Map pipeline): 5 integration tests against `freedoom1.wad` (E1M1 sector-closure + cross-episode parsing smoke), the rest unit tests using `SyntheticMapBuilder`-built lumps.
