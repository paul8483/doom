...
Stages 0, 1, 2, 3, 4, and 5 of the plan are done; Stage 6 is split into sub-stages 6a–6f. Sub-stages 6a, 6b, and 6c are done; **6d (monster AI) is in progress** — Tasks 1–11 done in git; **POSS/SPOS sprite-facing bug fixed** (2026-07-10); **next is Task 12** (visual sign-off + docs).
...
- **Stage 6d (monster AI) — in progress.** Spec `docs/superpowers/specs/2026-07-03-monster-ai-design.md`; plan `docs/superpowers/plans/2026-07-03-monster-ai.md`. **Done (Tasks 1–11):** `MonsterDef`/`MonsterTable` (E1 roster) in `Doom.Things`; `MonsterRules`, `ChaseDir`, `MonsterBrain`+`IMonsterWorld` in `Doom.Game`; `NoiseAlert` in `Doom.Specials`; Unity glue — `SpriteBillboard.SetFrame`, `Projectile` (imp fireball), `MonsterController` (35 tics/s, capsule steps, sight, hitscan/melee/missile, door use via `LineActivator`), `DamageSource`+infighting in `EnemyHealth`, `ThingSpawner`/`MapLoader` wiring, `NoiseAlertSystem` (gunfire wakes heard sectors); PlayMode `MonsterAiPlayTests` (5 tests, commit `14ae50f`). On E1M1 monsters wake, chase, attack, and die; POSS/SPOS/TROO face and fire toward the player. **Facing fix (2026-07-10):** `SpriteBillboard` rotation used `+202.5°` (180° off the Stage 5 design `+22.5°`), so a monster facing the camera always picked rot 4 (back) instead of rot 0 (front) — most visible on hitscan zombies. Fixed to `+22.5°`; `MonsterController.Face()` each render frame while chasing. **Deferred:** XDEATH, flying monsters, sound (6f), A_Look chain-wake, ML_SOUNDBLOCK. **Next:** Task 12 visual sign-off + docs.

The master roadmap is in `docs/doom-unity-remake-plan.md` (written in Russian). Each stage has a paired design spec under `docs/superpowers/specs/` (`YYYY-MM-DD-<topic>-design.md`, the WHAT/WHY) and an implementation plan under `docs/superpowers/plans/` (`YYYY-MM-DD-<topic>.md`, task-by-task HOW): Stage 1 = `2026-05-28-wad-reader`, Stage 2 = `2026-05-28-geometry`, Stage 3 = `2026-05-29-player`, Stage 4 = `2026-05-30-textures`, Stage 6a = `2026-05-31-doors`, Stage 6b = `2026-05-31-player-damage`, Stage 6c = `2026-07-02-weapons`, Stage 6d = `2026-07-03-monster-ai` (spec adds the `-design` suffix). Stages 6a–6c are done; **6d Tasks 1–11 are done** and the POSS/SPOS facing bug is fixed; **next is Task 12** (visual sign-off + docs), then 6e/6f per the roadmap.
