...
**Предыдущие этапы:** 0–5 завершены; под-этапы 6a–6d завершены
...
- **Рюкзак (8):** удваивает текущие максимумы `AmmoModel` (bullets 200→400,
  shells 50→100) и сразу даёт по одной «обойме» каждого *имеющегося* типа
  (+10 пуль, +4 дроби). Ракеты/ячейки не заводим — оружия под них нет.
...
- `HealthModel` — добавить:
  ...
  - `bool GiveArmorBonus(int amount)` — +N к броне ≤200; если `ArmorType==None`
    → Green.
- `KeyInventory` — биты/набор по `Doom.Specials.KeyKind` (card/skull);
  `Give(KeyKind)`, `Has(KeyKind)` (для `Any` — есть ли хоть один ключ),
  `Reset()` / сохранение при респавне решается в глю (см. ниже).
