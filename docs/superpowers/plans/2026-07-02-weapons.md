...
- [ ] **Step 4: Run test to verify it passes** — filter `Doom.Game.Tests.DoomRandomTests`, expected 3/3 PASS.

- [ ] **Step 5: Commit** — `git add Assets/Scripts/Game/DoomRandom.cs* Assets/Tests/EditMode/Game/DoomRandomTests.cs*` (включая .meta, которые сгенерирует Unity), `git commit -m "Stage 6c: DoomRandom - P_Random port with rndtable"`.
...
- [ ] **Step 2: Run to verify FAIL** (filter `Doom.Game.Tests.AmmoModelTests`).
...
- [ ] **Step 5: Commit** — `git commit -m "Stage 6c: AmmoModel with DOOM start/max values"`.
...
- [ ] **Step 4: Run to verify PASS.**
- [ ] **Step 5: Commit** — `git commit -m "Stage 6c: WeaponDef/WeaponTable with p_pspr.c timings"`.
...
- [ ] **Step 4: Run to verify PASS.**
- [ ] **Step 5: Commit** — `git commit -m "Stage 6c: WeaponLoadout - ownership, select, P_CheckAmmo order"`.
...
- [ ] **Step 4: Run to verify PASS** (filter `Doom.Game.Tests` — заодно все Game-тесты).
- [ ] **Step 5: Commit** — `git commit -m "Stage 6c: HitscanRules - DOOM damage rolls and spread"`.
...
- [ ] **Step 2: Run to verify FAIL** (filter `Doom.Things.Tests.MonsterDataTests`): нет полей `Health`/`CorpseFrame`.
...
- [ ] **Step 4: Run to verify PASS** (оба теста; второй — интеграция с WAD).
- [ ] **Step 5: Run** полный EditMode без фильтра — существующие Things-тесты не сломаны.
- [ ] **Step 6: Commit** — `git commit -m "Stage 6c: ThingDef health/corpse frame for E1 monsters"`.
...
- [ ] **Step 1: `SpriteBillboard.SetStaticFrame`**
...
- [ ] **Step 3: навеска в `ThingSpawner.SpawnAll`**
...
- [ ] **Step 4: Компиляция** — прогнать EditMode полный (быстрый компайл-гейт): 0 ошибок, все тесты зелёные.
- [ ] **Step 5: Commit** — `git commit -m "Stage 6c: EnemyHealth + corpse frame on billboards"`.
...
- [ ] **Step 1: Implement** (логика тривиальна и Unity-only; проверяется PlayMode-тестом Task 12 косвенно и глазами)
...
- [ ] **Step 2: Компиляция** — EditMode полный прогон, 0 ошибок.
- [ ] **Step 3: Commit** — `git commit -m "Stage 6c: HitEffect - PUFF/BLUD short-lived billboards"`.
...
- [ ] **Step 2: Компиляция** — EditMode полный, 0 ошибок.
- [ ] **Step 3: Commit** — `git commit -m "Stage 6c: PlayerWeapons - input, hitscan volleys, pickups"`.
...
- [ ] **Step 2: Глазная проверка формулы размещения** — после Task 11 (когда всё
...
- [ ] **Step 3: Компиляция** — EditMode полный, 0 ошибок.
- [ ] **Step 4: Commit** — `git commit -m "Stage 6c: WeaponView - viewmodel frames, flash, bob"`.
...
- [ ] **Step 2: навеска в `ThingSpawner.SpawnAll`** (после блока EnemyHealth):
...
- [ ] **Step 3: `MapLoader.SpawnPlayer`** — после блока `rider` добавить:
...
- [ ] **Step 4: `PlayerHud`** — поле `PlayerWeapons weapons` + `public void
...
- [ ] **Step 5: `PlayerDeathHandler`** — добавить `public event System.Action
...
- [ ] **Step 6: Компиляция + полный EditMode** — 0 ошибок, все зелёные.
- [ ] **Step 7: Commit** — `git commit -m "Stage 6c: pickups, MapLoader wiring, HUD ammo, respawn reset"`.
...
- [ ] **Step 2: Run PlayMode** (filter `Doom.Stage3.PlayTests.WeaponPlayTests`) — 3/3 PASS. Если pickup-тест падает на триггере (CC не генерирует OnTriggerEnter) — план Б: в `ThingPickup` заменить триггер на дистанционную проверку в `Update` (расстояние до игрока < radius), это тоже DOOM-way.
- [ ] **Step 3: Run ВСЁ** — полный EditMode и полный PlayMode без фильтров: старые тесты не сломаны.
- [ ] **Step 4: Commit** — `git commit -m "Stage 6c: PlayMode tests - kill, ammo autoswitch, pickup"`.
...
- [ ] **Step 1: Глазная проверка** — прогнать PlayMode `StairCaptureTests`, открыть `Logs/stair-spawn.png`: viewmodel пистолета по центру снизу, HUD показывает `AMMO 50`. Если оружие съехало — см. Task 10 Step 2. Затем интерактивно (или попросить пользователя): выстрелить в зомби, увидеть кровь/труп, подобрать дробовик.
- [ ] **Step 2: `CLAUDE.md`** — в списке под-этапов добавить абзац Stage 6c (по образцу 6b: сборки, компоненты, что отложено); обновить «Project status» (6c done) и счётчики тестов на фактические из финального прогона (ожидаемо ~159 EditMode / ~15 PlayMode — сверить с XML).
- [ ] **Step 3: `docs/doom-unity-remake-plan.md`** — пометить «Оружие и стрельба ✅ (под-этап 6c)» и добавить абзац «Под-этап 6c ✅» по образцу 6a/6b со ссылками на спеку/план.
- [ ] **Step 4: Отметить все чекбоксы в этом плане.**
- [ ] **Step 5: Финальный прогон** полного EditMode + PlayMode, оба зелёные (числа для CLAUDE.md берутся отсюда).
- [ ] **Step 6: Commit** — `git commit -m "Stage 6c done: weapons & shooting - docs, plan checkboxes"`.
