...
- ~~Бочки (2035) стреляются, но без HP/взрыва — отложено (Health=0 у не-монстров).~~
  **Сделано 2026-07-11:** `BarrelRules` + `RadiusDamageRules` (P_RadiusAttack 128/128),
  `BarrelExplosion` (BEXP + DSBAREXP), `ThingTable` HP=20, chain splash, no CountKill/blood.
  EditMode `RadiusDamageRulesTests` + `BarrelDataTests`; PlayMode `BarrelPlayTests` ×2.
