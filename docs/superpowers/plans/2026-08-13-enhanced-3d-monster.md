...
## BOSS — пятый монстр (барон), трек закрыт ✅ 2026-08-15

8 кадров A–H (атака E–F–G, зелёный шар вылетает на G, боль H); высоты
69/72/69/72/74/73/74/73. Проекция эскизов с `--head-from-bake 0.8`
(череп-морда), без skin/accent-фиксов — серо-стальная броня + зелёные
мышцы + красный шар на запястье легли с первого прохода. Таргетные
тесты 4/4; полные сьюты на закрытии: EditMode **624/624**, PlayMode
**167/167**; Windows build Success, **250 МБ** (5 монстров × 7–8 мешей).
Трек закрыт решением пользователя по панелям; отдельный интерактивный
E1M8-гейт на момент закрытия не проводился. Коммит `f4c82d1`.

**Итог трека:** все пять уникальных E1-монстров — POSS, SPOS, SARG
(спектр остаётся MF_SHADOW-билбордом), TROO, BOSS — стоп-моушен 3D в
Enhanced. Устоявшийся рецепт: пользовательские v2-редравы кадров →
TRELLIS.2 (общий seed) → doomify-меш (`--palette-lump` кадра A) →
`project_hint_texture.py` (текстура из эскиза; TRELLIS-бейк только для
головы) → runtime-таблица `ExperimentalMonsterModel.Sets`. Открытые
хвосты: 3D-кадры смерти, пере-роллы (SARG F≈G, SPOS-вставки),
производительность (5 монстров × 7–8 мешей × 40k tris), XDEATH.

## TROO — четвёртый монстр (имп-серпентипед) ✅ 2026-08-14

Первый монстр целиком на проекционном пайплайне. 8 кадров A–H (атака
E–F–G, файрбол вылетает на G, боль H); высоты 60/62/60/62/62/61/64/63.
Наработки `project_hint_texture.py` за волну: `--side-accent-fix`
(золото брюха — только фронтальным поверхностям по нормалям, знак
проверен знаковым объёмом; бока/спина ловили жёлтые пятна),
`--head-from-bake` (проекция размазывала гребень по всей макушке — в
походке читалось как «две головы»; голова остаётся на геометрически
выровненном TRELLIS-бейке), eye-boost из сырого бейка (жёлтые глаза
умирали в 256px-квантизации, морда не читалась — имп ДВАЖДЫ был
доложен «идущим спиной», хотя ориентация была верной; yaw offset у
всех монстров одинаково 0), `--lower-skin-fix` стал опциональным
(SPOS-правило, имп кожаный целиком). Вердикт «теперь выглядит хорошо».
Коммит `f08e8fe`.
