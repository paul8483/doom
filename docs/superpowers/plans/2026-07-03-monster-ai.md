...
**Статус (2026-07-10):** Stage **6d ✅** — Tasks **1–12** done. Task 11 = `14ae50f`. **Facing-баг POSS/SPOS ✅** — причина: `SpriteBillboard` использовал `+202.5°` вместо `+22.5°` из спеки Stage 5 (сдвиг на 180° → всегда rot 4 = спина при взгляде на камеру). Исправлено; `Face()` каждый render-кадр в Chase/Attack/Pain. Task **12** — визуальная приёмка подтверждена пользователем на E1M1; docs обновлены.
...
- [x] **Step 1: Глазная проверка.** Прогнать `StairCaptureTests` (харнесс видит мир, монстров и снаряды — не видит только OnGUI), посмотреть PNG: монстры в походных кадрах с ротациями. Затем попросить пользователя интерактивно: разбудить комнату выстрелом, увидеть погоню/атаки/фаербол/драку/смерти, монстра, открывшего дверь. ✅ подтверждено пользователем 2026-07-10.
- [x] **Step 2: `CLAUDE.md`** — абзац Stage 6d по образцу 6a–6c (сборки, компоненты, отложенное), статус, счётчики тестов из финальных XML.
- [x] **Step 3: `docs/doom-unity-remake-plan.md`** — «ИИ врагов ✅ (под-этап 6d)» + абзац по образцу.
- [x] **Step 4: Чекбоксы в этом плане.**
- [x] **Step 5: Финальный прогон** полного EditMode + PlayMode; числа для CLAUDE.md — отсюда. (193/193 + 20/20; повторный batchmode отложен — Unity Editor был открыт на приёмке.)
- [x] **Step 6: Commit** — `git commit -m "Stage 6d done: monster AI - docs, plan checkboxes"`.
