...
Превратить «прогулку» в игру. Реализуется поэтапно, каждый пункт — отдельная веха. Этап разбит на под-этапы 6a–6f; 6a (двери и интерактивные секторы), 6b (урон игроку и HP), 6c (оружие и стрельба) и **6d (ИИ врагов) завершены**; далее 6e (подбираемые предметы) и 6f (звук).
