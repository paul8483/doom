...
Создать собственный движок-ремейк оригинального DOOM на Unity и C# — именно **REMAKE**, а не очередной порт. Проект отвечает на вопрос «зачем переписывать DOOM на Unity» двумя режимами:

- **Classic** — бит-точное воспроизведение оригинала: на экране только WAD-данные. Эта фаза завершена — играбельный DOOM на Unity состоялся.
- **Enhanced** — то, ради чего выбран Unity: улучшенная графика с сохранением стиля оригинала (свет, туман, объёмные объекты).

Оригинальный контент (карты, текстуры, спрайты, звуки) берётся из WAD-файлов как источник данных и идентичности. Вся отрисовка, физика и игровая логика пишутся с нуля средствами Unity. Карты и геймплей-данные никогда не создаются вручную и не генерируются ИИ. Для Enhanced-представления с 2026-08-07 допускаются **ИИ-производные ассеты** (перерисовки и 3D-модели, сгенерированные из WAD-спрайтов) — по доктрине `docs/superpowers/specs/2026-08-07-enhanced-ai-derived-assets-design.md`: каждый ассет привязан к своему WAD-лампу, проходит интерактивный гейт аутентичности и обратим; WAD не распаковывается.
...
replacement assets не добавляются *(ограничение действовало на момент Stage 8;
снято 2026-08-07 доктриной AI-derived assets — см. Этап 10)*. Classic остаётся
режимом по умолчанию и эталоном визуальной регрессии.
...
первого лица; этап 2 — contrast gate (`Scale8XContrastGated`), чтобы не
размывать намеренные высококонтрастные детали (крест аптечки, гем брони).
...
Поиск «более радикального» улучшения предметов — плоские апскейлы не давали
вау-эффекта:

- **Pixel Extrusion ❌** (2026-08-05, `pixel-extrusion`) — равномерное
  выдавливание текселей провалило Gate 1 на не-коробочных предметах.
- **Parametric Pickups ❌** (2026-08-06, `parametric-pickups`) —
  per-item параметрический Blender-цикл слишком медленный, остановлен.
- **TRELLIS.2 image-to-3D ✅** (2026-08-06+) — сработал; вырос в Этап 10.
...
## Этап 10. Enhanced AI-derived assets (TRELLIS.2 3D) 🚧

Новая фаза Enhanced: объёмные объекты взаимодействия. Вывод релиза 1.0.3 —
вау-эффект живёт в предметах, на которые смотришь в упор, а не в стенах.
Доктрина зафиксирована в
`docs/superpowers/specs/2026-08-07-enhanced-ai-derived-assets-design.md`:
WAD остаётся источником истины и идентичности, Enhanced — производный слой;
пайплайн native-спрайт → перерисовка (GPT-5.6 SOL) → TRELLIS.2 → интерактивный
гейт; центральный критерий — **баланс аутентичности** (улучшение, вписанное в
пиксельный мир DOOM, а не «Heroes 3 в 3D»).

**Сделано (Accepted, 2026-08-06/07):** `MEDIA0`, `BON1A0`, шесть оружейных
pickups, `COLUA0` лампа, `BAR1A0` бочка (с откатом к BEXP-билборду при
взрыве), деревья `TRE2A0`/`SMITA0`. Classic везде сохраняет билборды;
...
кадры; строжайший гейт аутентичности) и ИИ-музыка (будущее, та же доктрина).

**Результат этапа:** Enhanced, в котором предметы, декорации (и со временем
монстры) объёмны, но читаются как объекты того же пиксельного мира; игрок
выбирает степень отхода от оригинала (Classic / Enhanced 2D / Enhanced 3D).
