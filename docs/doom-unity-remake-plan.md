...
Превратить «прогулку» в игру. Реализуется поэтапно, каждый пункт — отдельная веха. Этап разбит на под-этапы 6a–6f; 6a (двери и интерактивные секторы), 6b (урон игроку и HP) и 6c (оружие и стрельба) завершены; **6d (ИИ врагов) в работе** — Tasks 1–10 сделаны, Task 11 (PlayMode) WIP.

- Здоровье, броня, урон. ✅ (под-этап 6b)
- Оружие и стрельба. ✅ (под-этап 6c)
- ИИ врагов (преследование, атака, смерть). 🔄 (под-этап 6d, Tasks 1–10 ✅)
...
**Под-этап 6d (ИИ врагов) 🔄.** Четыре монстра E1 (POSS/SPOS/TROO/SARG): FSM в `Doom.Game` (`MonsterBrain`, `MonsterRules`, `ChaseDir`), данные в `Doom.Things` (`MonsterDef`/`MonsterTable`), заливка шума `NoiseAlert` в `Doom.Specials`. Unity-глю: `SpriteBillboard.SetFrame`, `Projectile` (фаербол импа), `MonsterController` (движение, зрение, атаки, двери), `DamageSource`+infighting в `EnemyHealth`, навеска в `ThingSpawner`, `NoiseAlertSystem` (выстрелы будят сектор). **193 EditMode** (включая 31 новых из Tasks 1–6). **PlayMode Task 11** (`MonsterAiPlayTests`, 5 тестов) — WIP, не в коммите; стабилизировать в отдельной сессии. Отложено: XDEATH, летающие монстры, звук (6f), визуальная приёмка (Task 12). Дизайн — `docs/superpowers/specs/2026-07-03-monster-ai-design.md`; план — `docs/superpowers/plans/2026-07-03-monster-ai.md`.
